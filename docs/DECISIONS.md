...
- **Date:** 2026-06-30
- **Status:** Accepted; the LIBRARY tier + its unit test SHIPPED + verified (`tsc --noEmit` clean; `mapcheck.test.ts` **11 pass** — the 7 originals + 4 new; the shipped `door-audit.ts` gate re-runs BYTE-IDENTICAL at 0 stuck · 0 wrong-edge · 8 one-way). The GATE WIRING (enable the tier in `door-audit.ts` + `content-validate.ts` and waive the 2 generated city-unit doorsteps) is STAGED — deferred behind the in-flight ADR-134 balance pass, which is editing `content-validate.ts` (avoid the collision; flip it on when that settles).
