...
| **Mega-buildings are common in Brickton + a colossus landmark** — the HIGH-RISE DOWNTOWN (west of the avenue) stands 3 mega-towers whose tops run off-screen, and the STARFALL SPIRE colossus (`bldg_colossus_spire`, 226×524 ≈ 2.3 screens) is the far-east landmark you round on foot | ✅ 4 megas total; `art:buildings` shots `buildings_s15i`, `buildings_colossi` show the off-screen-top scale vs the hero (24×32 native / 96×128 runtime) |
...
> **Current supersession (ADR-143, 2026-07-13):** “unlanded” and “bindings
> deferred” describe the historical M20 catalog pour only. Chapter 8 now has
> production maps, quests, battle hooks, and save v25. Spore Antidote is consumed
> when it cures Mushroomized; Scroll of Calm uses the shared reusable-cure path;
> doctors also cure the status. The final Chapter 8 run results are recorded in
> `docs/CH8_PRODUCTION_VERIFICATION.md`, not retroactively in this M20 log.
...
| **THE THREE NEW-MECHANIC DECISIONS — historical M20 state** — (1) `heroResist`: volt+fire resist shipped as DATA before its later damage binding; (2) the Spice Box began as a flavor KEY before Ch.7 wiring; (3) the Scroll of Calm began as `reusable:true` DATA before Ch.8's cure binding | ✅ accurate for the M20 catalog commit; **superseded for current runtime by later production work, including ADR-143 for the Scroll/Mushroomized path** |
...
| **UNLANDED HELD (historical M20 scope)** — no maps*.ts / shops.ts / quests were touched by that catalog pour | ✅ accurate for M20; Chapters 6, 7, and 8 subsequently received production rollouts |
...
## S18 M25 (ADR-095) — LAND ENGLAND: Ch.3 "A Very Foggy Term", PART 1 (maps + encounters + shops)

The first UNLANDED chapter begins to ship — the §A6 Old-World content track OPENS, split on the Appendix-
tip-2 seam. PART 1 lands the WORLD (the maps, the 20 §A7 enemies, the shops); PART 2 (the story/boss half)
writes the §A11 dialogue beats + the Headmaster Mainframe + the five quests and FLIPS the manifest to
`shipped`. Verified headlessly (the WebGL canvas hangs `preview_screenshot`, ADR-059/060). No save
migration (Part 1 adds no persistent field). A device-playthrough row rides PART 2 (when Ch.3 is
completable end-to-end).

| Check | Result |
|---|---|
| **THE ENGLAND MAPS** (`src/data/maps_ch3.ts`) — Lucille (arrival), Foggybottom-on-Tyne (town), the Fog Road (moor), Wintermoor grounds, the Old Stones (Resonance Site) + the dungeon: 3 school floors + the dorm STEALTH WING + the boiler PSI-gate room. Each settlement declares its `area` (the M22 `fraktur` banner over the M25 skin); 3 §A4.5 picnics before the dungeon | ✅ all clear `map-quality` (reachability + door-landing) + `encounter-pressure` (grace + proximity); 108 maps (the 10 + the living-city auto-interiors); foggybottom/wintermoor banners + skins pinned |
| **THE 20 §A7 Ch.3 ENEMIES** (`src/data/enemies.ts`) — the seed six + the Flow Law mix (4 road/field · 3 dungeon · 2 social · 2 rare · 2 late · 1 set-piece), each a battle HOOK + an identity DROP (M24's `EnemyDef.drops`, economy-neutral) + a place-specific DEATH LINE; placed live across the maps' spawners + the dorm patrols. Dev-art: the 6 forged Ch.3 faces composed into `ENEMY_BATTLE_ART` (the `wear` gate), boot-registered | ✅ `34 enemies (§A7 Ch.1–3 + Bosses 1–2)` · `29 §A7 drops`; canon enemy manifest extended (ADR-017) |
| **HUSHED, the chapter's signature status** — `'hushed'` joins the §A7 move enum + a one-branch BattleScene apply (the engine already modelled a hushed hero); the Possessed Textbook's pop quiz silences your Vibe and drops the honey lozenge that cures it (cause → cure, §A7). The Tea Poltergeist's heal-allies mechanic is deferred to PART 2 | ✅ tsc + validate green; the textbook + the Invigilator inflict it in data |
| **TWO Ch.3 SHOPS** (`src/data/shops.ts`) — Boothe's Chemist & Teas (cures + tea-as-PP + food + moor gear) + the rationed Tuck Shop (Milo's gun ladder + his repaired-gizmo battle goods; the Gauss Lobber held back as the boss drop); the Ch.1–2 "exactly 4 shops" check generalises to a per-chapter allowlist | ✅ `6 shops`; every shelf real + priced + affordable on the Ch.3 Fortune Arc; keepers placed once |
| **§A11 voice** — damp-Tyne stone + institution-fear, one obsession per soul, the machine-fog kept plain (Hush-adjacent, never funny); flair rare + never on the shop surface (§A11.9); the groundskeeper's cold thermos seeds quest #8, the stones hum back at the locket (the PART-2 Heartlight foreshadow) | ✅ read out loud; the sincere undertones land (the dorm diary, the optimised Tuesdays) |
| **A STALE GATE RETIRED** — a concurrent S18 refactor removed costa_estrella's clubhouse prop but left the `links` gate requiring it; the obsolete check is retired (the door west to golf_resort is the live link) | ✅ validate green |
| `npm run validate` + `npx tsc --noEmit` + `npx vite build` + `npm run balance` | ✅ validator GREEN (0 fails) + tsc clean + `vite build` clean + balance read by eye (§A9 + fuel/ferry ladders sane; the Ch.3 boss TTK reads at the PART-2 flip) |
| Full `npx vitest run` | ⚠ the Ch.3 content passes its suites; the only failures (4) are the CONCURRENT, in-development `occupyCity` "Living-City Law (S18)" feature — Brickton/Puerto Sol at 71% enterable + the frozen-core door additions — independent of Ch.3 (foggybottom + wintermoor PASS the sweep) |

## S18 M26 (ADR-099) — LAND ENGLAND: Ch.3 "A Very Foggy Term", PART 2 (story + boss + the FLIP)

The SOUL half. The §A11 story beats, the Headmaster Mainframe, the five §A10 quests, the control/PSI/heal
systems, and THE FLIP (`CHAPTER_MANIFESTS['3']` → `'shipped'`, the validator's live Ch.3 assertions on, in
the same commit — ADR-047). Concurrent note: the user's depth-pass + living-city work took ADR-096/097/098
mid-flight, so this movement is **ADR-099**. Verified headlessly (the WebGL canvas hangs `preview_screenshot`,
ADR-059/060). No save migration — Milo's join is a party push + a flag; the awakening, trust beat,
`ch3_complete`, quest flags, and gate flags ride the ADR-015 ledger free.

| Check | Result |
|---|---|
| **THE STORY** (`src/data/dialogue.ts`) — the Lucille arrival through the machine-fog, Milo's greenhouse crash + JOIN (party of three), THE FIRST BORROW (Jay puppets the porter, the Trust Thread opens), the machine-fog reveal, the Mainframe beats (the Hush underneath never funny), the Repair tutorial → Defibrillator, Heartlight 3, `ch3_complete`. The Part-1 ambient lines seeded every payoff | ✅ read out loud; the sincere beats land (Milo's dad-ache, the half-step recoil, the XI going home, Mr. Stumps). Flair stays rare (one `{g:gear}`) |
| **BOSS 3 — HEADMASTER MAINFRAME** — promoted from the `summoner` draft to a LIVE boss enemy (`boss:true`, hp 1600, `mind_immune`, lvl 18, `weakness:['freeze']`) + a `BOSS_SCRIPTS` phase machine (open→summon 2 Prefect Drones; refill on bothSummonsDead forever; 40% overclock flavor). Spy + Vibe Freeze ×2 ride the §A7 weakness table (ADR-046). Draft removed from `DRAFT_BOSS_SCRIPTS`/`DRAFT_BOSS_IDS`; canon HP + boss-list + `ENEMY_BATTLE_ART` rows added | ✅ `35 enemies (Bosses 1–3)`; `phases.test.ts` +3 green (the real script: opens, refills forever, overclocks once; the freeze weak point on the enemy) |
| **FIVE §A10 QUESTS** — #7 Overdue (→ Library Card + First Edition), #8 The Groundskeeper's Cuppa (→ Thermos), + Return to Sender / The Penny Fog / The Last Over (the Flow-Law trio). Rewards reuse the live ch3 catalog (no new ITEMS row); two are caller+flag (ADR-073). Each adds a finale CALLER; givers mirror `tallyBeat`; "find" steps fire as active-quest walk triggers | ✅ `13 quests`; the §A10 validator manifest extended both directions; `cricket_captain` + `wm_umpire` placed; non-missable + retry-safe |
| **SYSTEMS LIVE** — the control system (scripted Puppet beat + Milo's Clicker + the `fleet_road` marker; battle Mind Warp/Spy/Repair/Bottle-Rocket on join; the free-roam wheel is the ADR-068 follow-up), the Tea Poltergeist's deferred `'mend'` heal-allies move (schema + BattleScene + the validator pin), and the §A4.11 PSI freeze-cast at `wintermoor_coolant` (taught-first via `availableAbilities`/`canClearGate`, non-missable, retry-safe) | ✅ tsc + validate green; the freeze cast opens the fog-engine beat + a Broken-Gizmo salvage |
| **THE FLIP (ADR-047)** — `status:'shipped'`, `dungeon.maps` (5 interiors), `maps` (5 overworld), all 5 `quests`; `site`/settlement `style` drop. The shipped branch switches on (maps live, boss live + flagged + scripted, quests both-directions) | ✅ `10 chapter manifests (3 shipped · 7 unlanded)`; the draft-claimed check stays consistent |
| **THE LINKS CLUBHOUSE DOOR** (the live bug, ADR-051 way) — `LANDMARK_FACADE_SPRITES` routes the golf clubhouse/gatehouse/mansions through the texture-derived `facadeSolids`/`facadeDoorBox` rebuild, so collision == drawn footprint minus the doorway and the entrance sits at the door's mouth (no walk-through walls, no too-deep trigger). §B4 laws intact | ✅ `maps.test.ts` +3 green; verified by `window.mfSolids()` reasoning (the 8×3 clubhouse's full body now collides; City Hall already `bldg_`) |
