/**
 * tools/sync-ch10-tiles.ts — append (or refresh) the Ch.10 region tiles
 * (aurora_/lani_/mars_ × {ground,path,wall}) as the trailing columns of the runtime
 * world-tile strip (assets/art/world/otterbrook_tiles_16.png).
 *
 ...
 * tools/sync-romania-tiles.ts: keep every existing column byte-for-byte (never re-pack)
 * and (re)write only the trailing N columns from each tile's procedural fallback painter
 * (the boot source of truth). The real Ch.10 look comes from {Aurora,Lani,Mars}_tiles_16.png
 * at runtime (partial-override strips wired in spritegen/authored.ts); these fallback columns
 * only show if those PNGs fail to load. Re-runnable/idempotent.
 *
 * Run:  npx vite-node tools/sync-ch10-tiles.ts
 */
...
const NEW_TILES = [
  'aurora_ground', 'aurora_path', 'aurora_wall',
  'lani_ground', 'lani_path', 'lani_wall',
  'mars_ground', 'mars_path', 'mars_wall',
] as const;

// the Ch.10 tiles must be the LAST N entries of TILESET, in this order
...
const keepCols = firstNew; // every tile before the first Ch.10 tile keeps its column
...
// 1) copy every existing column byte-for-byte (preserves authored fence/melt_ice/minimus/china/romania tiles)
...
console.log(`synced ${NEW_TILES.length} Ch.10 tiles (cols ${keepCols}..${TILESET.length - 1}) -> ${STRIP} (${out.w}x${out.h})`);
