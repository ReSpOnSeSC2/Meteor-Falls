/**
 * tools/sync-romania-tiles.ts — append (or refresh) the Ch.9 ROMANIA (Valea Stelelor) region
 * tiles (romania_ground / romania_path / romania_wall) as the trailing columns of the runtime
 ...
 * (the boot source of truth). The real Romania look comes from Romania_tiles_16.png at runtime
 ...
 *
 * Run:  npx vite-node tools/sync-romania-tiles.ts
 */
...
const NEW_TILES = ['romania_ground', 'romania_path', 'romania_wall'] as const;

// the Romania tiles must be the LAST N entries of TILESET, in this order
...
const keepCols = firstNew; // every tile before the first Romania tile keeps its column
...
// 1) copy every existing column byte-for-byte (preserves authored fence/melt_ice/minimus/china tiles)
...
console.log(`synced ${NEW_TILES.length} Romania tiles (cols ${keepCols}..${TILESET.length - 1}) -> ${STRIP} (${out.w}x${out.h})`);
