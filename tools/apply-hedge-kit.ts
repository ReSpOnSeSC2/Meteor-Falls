/**
 * tools/apply-hedge-kit.ts — install the WORLD-OVERHAUL HEDGE-WALL autotile
 ...
 *
 * Source: one ChatGPT 4×4 magenta-gridded sheet of hedge connectivity pieces:
 *   assets/art/masters/world/hedge-wall-source.png
 *
 * What it does:
 ...
 *        rim    = the round POST cell, filled to opaque — a lit leaf rim on every edge
 *        hedge_<mask> = base with the rim composited onto each OPEN edge (bit CLEAR).
 *        hedge_0 (isolated) = the round post over grass (a lone rounded bush).
 *      bits: 1=N 2=E 4=S 8=W (a SET bit = neighbour is hedge = no rim there).
 *   3. Grows the strip to TILESET.length×64 and writes each hedge_<mask> column at
 *      its imported TILESET index (never hardcoded).
 *   A .pre-hedge.bak.png of the strip is written beside it first.
 *   Also dumps tools/_hedge_proof.png — a little hedge maze assembled from the 16
 *   composed tiles (autotiling proof).
 *
 *   npx tsx tools/apply-hedge-kit.ts
 */
...
const SRC = `${ROOT}assets/art/masters/world/hedge-wall-source.png`;
const PROOF = `${ROOT}output/hedge_proof.png`;
const CELL = 64;
...
/** overlay the rim source's leaf rim onto base's edge on the given side (opaque in/out) */
...
/** mean opaque colour of a cell (its dominant hedge green) */
...
if (xs.length !== 4 || ys.length !== 4) throw new Error(`hedge source grid ${xs.length}×${ys.length}, want 4×4`);
const cells: Uint8Array[] = [];
for (const ry of ys) for (const rx of xs) cells.push(scaleCellAlpha(src, rx, ry));

// ---- the live strip + the grass_a base cell (so hedge gaps read as ground)
...
const hedgeGreen = meanColor(cells[INTERIOR]);
const base = over(grass, cells[INTERIOR]); // opaque dense hedge, gaps → grass
const rimSrc = over(solidFill(hedgeGreen), cells[POST]); // lit leaf rim on all edges, opaque
const post = over(grass, cells[POST]); // a lone rounded bush (grass in the corners)
...
const BAK = STRIP.replace(/\.png$/, '.pre-hedge.bak.png');
...
for (let m = 0; m < 16; m++) {
  const x0 = idxOf(`hedge_${m}`) * CELL, cell = tiles[m];
  for (let y = 0; y < CELL; y++) for (let x = 0; x < CELL; x++) {
  ...
  console.log(`  hedge_${m} <- column ${idxOf(`hedge_${m}`)}`);
}
fs.writeFileSync(STRIP, encodePng(grown));
console.log(`strip written: ${grown.w / CELL} columns (TILESET.length=${TILESET.length})`);

// ---- PROOF: assemble a little hedge maze from the composed tiles (same mask rule
...
for (let ty = 0; ty < MH; ty++) for (let tx = 0; tx < MW; tx++) {
  const isH = (x: number, y: number): boolean => y >= 0 && y < MH && x >= 0 && x < MW && MAZE[y][x] === 'H';
  const cell = MAZE[ty][tx] === 'H'
    ? tiles[(isH(tx, ty - 1) ? 1 : 0) | (isH(tx + 1, ty) ? 2 : 0) | (isH(tx, ty + 1) ? 4 : 0) | (isH(tx - 1, ty) ? 8 : 0)]
    : grass;
  for (let y = 0; y < CELL; y++) for (let x = 0; x < CELL; x++) {
