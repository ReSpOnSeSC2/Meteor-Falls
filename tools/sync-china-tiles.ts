/**
 * tools/sync-china-tiles.ts — append (or refresh) the PKG-15 §1 CHINA (Lotus Harbor) region
 * tiles (china_ground / china_path / china_wall) as the trailing columns of the runtime
 ...
 * tools/sync-minimus-tiles.ts: keep every existing column byte-for-byte (never re-pack)
 * and (re)write only the trailing N columns from each tile's procedural fallback painter
 * (the boot source of truth). The real China look comes from China_tiles_16.png at runtime
 * (a partial-override strip wired in spritegen/authored.ts); these fallback columns only
 * show if that PNG fails to load. Re-runnable/idempotent.
 *
 * Run:  npx vite-node tools/sync-china-tiles.ts
 */
...
const NEW_TILES = ['china_ground', 'china_path', 'china_wall'] as const;

// the China tiles must be the LAST N entries of TILESET, in this order
...
const keepCols = firstNew; // every tile before the first China tile keeps its column
...
// 1) copy every existing column byte-for-byte (preserves authored fence/melt_ice/minimus tiles)
...
console.log(`synced ${NEW_TILES.length} China tiles (cols ${keepCols}..${TILESET.length - 1}) -> ${STRIP} (${out.w}x${out.h})`);
